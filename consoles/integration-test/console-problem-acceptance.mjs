...
export async function verifyRequestProblemSurfaces(browser) {
  for (const locale of ['zh-CN', 'en-US']) {
    const labels =
      locale === 'zh-CN'
        ? { heading: '登录 SaaS Forge', email: /^邮箱/, password: /^密码/, error: '错误代码：' }
        : {
            heading: 'Sign in to SaaS Forge',
            email: /^Email/,
            password: /^Password/,
            error: 'Error code: ',
          };
    for (const [host, application] of [
      ...
      await context.addInitScript((locale) => {
        localStorage.setItem('sf:ui:locale', locale);
      }, locale);
      try {
        ...
        await page.getByRole('heading', { name: labels.heading, exact: true }).waitFor();
        ...
        for (const [kind, code] of [
          ...
          await page.getByLabel(labels.email).fill('fault-only@example.test');
          // 此凭据仅发送到上面的故障处理器；不尝试登录真实账号。
          await page.getByLabel(labels.password).fill('fault-only-input');
          await page.getByLabel(labels.password).press('Enter');
          await page
            .getByRole('alert')
            .filter({ hasText: `${labels.error}${code}` })
            .waitFor();
          ...
          assert.equal((await page.getByLabel(labels.password).inputValue()).length, 0);
          assert.equal(
            await page.getByRole('heading', { name: labels.heading, exact: true }).count(),
            1,
          );
          assert.equal(
            await page.evaluate(
