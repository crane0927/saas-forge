const addColorAlpha = (color: string, opacity: number) => `color-mix(in srgb, ${color} ${opacity * 100}%, transparent)`;
const transformColorWithOpacity = (color: string, opacity: number, background: string) => `color-mix(in srgb, ${color} ${opacity * 100}%, ${background})`;
