...
describe.each(['zh-CN', 'en-US'])(
  'same-Origin session coordination through AuthenticationRuntime %s',
  (locale) => {
        ...
        const tab = runtime(
          ...
          (input, init) =>
            Promise.resolve().then(() => {
              ...
              keys.push(new Headers(init?.headers).get('Idempotency-Key'));
              if (keys.length === 2)
                return problem(503, 'TOKEN_REVOCATION_STATUS_UNAVAILABLE', '2');
              return Response.json({
                contextState: 'ACCESS_TOKEN_ISSUED',
                accessToken: `token-${String(keys.length)}`,
                tokenType: 'Bearer',
    ...
    it.each(['preflight', 'replay', 'switch-refresh'])(
      ...
      async (operation) => {
        ...
        if (operation === 'switch-refresh')
          ...
          await first.client.getOAuthClient({ clientId: '018f1f2e-7b5a-7c42-8c91-2b3d4e5f6076' });
    ...
    it.each([
      ...
      'falls back without replaying a session mutation when %s is unavailable',
      async (failure) => {
        const origin = browserOrigin();
        ...
        if (failure === 'locks-getter')
          Object.defineProperty(realm, 'navigator', { get: unavailable });
    ...
    it('ends the restricted session in every tab after the initial password change', async () => {
      ...
      const second = runtime(origin.realm(), fetch);
      await Promise.all([first.recover(), second.recover()]);
      expect(origin.values.get('sf:session:https://api.example.test:PLATFORM:generation')).toBe(
        '2',
      );
      await Promise.all([
