...
test('matches exact Password Setup pathnames with queries while keeping other Tenant paths on Vite', async (t) => {
  ...
  assert.equal(
    (await fixture.request('/api/v1/auth/password-setups?fixture=1', { method: 'POST' })).status,
    409,
  );
  for (const requestPath of [
    '/',
    ...
    assert.equal((await fixture.request(requestPath)).body, 'Tenant SPA', requestPath);
  }
});
...
test('every formal Tenant path and API Host follows the active Gateway file without restarting Edge', async (t) => {
  ...
  for (const hostname of ['gateway', 'host.docker.internal', 'gateway']) {
    ...
    for (const requestPath of [
      ...
      assert.equal(response.headers['x-fixture-upstream'], hostname, requestPath);
    }
    assert.equal(
      (
...
test('rejects unapproved Hosts before resolving or forwarding any Password Setup request', async (t) => {
  ...
  for (const host of [
    ...
    assert.equal((await fixture.request('/password-setup', { host })).status, 421, host);
  }
});
...
async function edgeFixture(t, { dynamic = false } = {}) {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'sf-password-setup-edge-'));
  ...
  const tenantServer = createServer((incoming, outgoing) => outgoing.end('Tenant SPA'));
  ...
  if (dynamic) {
    ...
    await writeFile(targetFile, JSON.stringify({ hostname: 'gateway', port: 8080 }));
  }
      ...
      return new Promise((resolve, reject) => {
        const outgoing = request(
          ...
          (incoming) => {
            ...
            incoming.on('end', () =>
              resolve({ status: incoming.statusCode, headers: incoming.headers, body: content }),
            );
          },
        );
        outgoing.on('error', reject);
