...
[MVP 开发计划](16-mvp-development-plan.md)第 1 阶段的共享组件测试、无障碍、稳定状态视觉快照与 Playwright 事项，以共享测试基线为独立交付范围：覆盖现有共享组件、认证 Runtime、React Shell 的交互状态，以及两个 Console 的代表性真实路径。复用已有测试设施，明确覆盖矩阵、补齐矩阵内缺口，并提供真实运行证据。

各阶段的完整业务浏览器闭环仍由对应阶段验收；共享测试基线通过不代表这些业务闭环完成。本项继续遵循 [Design System 规范](25-design-system.md)的无障碍、主题与视觉要求，以及 [Console 国际化基线](29-console-internationalization.md)的双语和状态保持要求。真实产品浏览器测试复用隔离的全新 Compose 数据卷，不清理开发者已有环境。
...
保留已有视觉覆盖，并补齐下列稳定状态；每类状态覆盖 `zh-CN` / `en-US` 与浅色 / 深色的完整组合，至少包含桌面与窄屏。
...
布局继续覆盖既有 `1440px`、`1280px`、`768px`、`390px`、`360px` 五个视口，并验证相当于 `320 CSS px` 可用宽度下的阅读与操作。截图固定数据并等待字体、素材和布局稳定，不比较随机动画中间帧。自动无障碍检查与真实键盘验证继续遵循 Design System 的 WCAG 2.2 AA 工程验收基线，不以截图或自动扫描替代交互验证。
