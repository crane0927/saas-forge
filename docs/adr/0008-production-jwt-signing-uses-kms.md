...
生产 KMS/HSM 适配器尚未实现：当前只有 `PemJcaJwtSigningAdapter`，由 `security.jwt.signing.adapter=pem-jca` 条件装配，仓库不含任何厂商 KMS SDK。`prod` 缺少外部适配器 Bean 时拒绝启动，当前 Compose 验收只覆盖开发签名。适配器缺失的边界与后续接入要求见[安全设计](../12-security-design.md)；本文关于 `kid`、JWKS 发布顺序、轮换窗口与撤销集合的规则在适配器接入后继续适用。
