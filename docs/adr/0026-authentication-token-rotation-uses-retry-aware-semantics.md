...
> 新协议说明：[ADR 0052](0052-unified-console-authentication-uses-versioned-session-protocol.md) 的统一会话协议保留本文 Refresh 轮换与一次替代恢复规则；退出改为持久化可恢复流程，撤销整个当前 Family 的全部未过期 JTI。下文“登出不使用通用 HTTP 幂等记录”与旧登出行为描述仅适用于现行 v1，不阻止新协议定义专用退出恢复。
