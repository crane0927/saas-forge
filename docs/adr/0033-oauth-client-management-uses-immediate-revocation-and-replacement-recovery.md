...
OAuth Client 管理只接受 Platform Admin 的 Platform User Access Token，公开创建只产生 `RUNTIME_SERVICE`，三个 `RESERVED_SERVICE` 仍由部署流程按固定服务身份和 Scope 管理。Client 的名称、类型与 Scope 创建后不可变；MVP 只增加单 Client 详情、创建、Secret 轮换、Secret 签发恢复和不可逆吊销，不提供修改、恢复 Client 或单独吊销某把 Secret。Client 列表、操作记录与凭据状态查询随后作为增量交付，见 [OAuth Client 读取验收记录](../acceptance/issue-178-oauth-client-reads.md) 与[管理规格](../22-oauth-client-credentials-management.md)。
