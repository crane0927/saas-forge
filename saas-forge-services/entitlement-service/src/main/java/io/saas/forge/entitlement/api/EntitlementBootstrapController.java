...
import io.saas.forge.entitlement.application.bootstrap.QuotaDefinitionOperation;
import io.saas.forge.entitlement.application.bootstrap.PlanOperation;
import io.saas.forge.entitlement.application.subscription.SubscriptionOperation;
import io.saas.forge.entitlement.contract.model.SubscriptionOperationPage;
import io.saas.forge.entitlement.contract.model.SubscriptionOperationRecovery;
import io.saas.forge.entitlement.contract.model.PlanOperationRecovery;
import io.saas.forge.entitlement.contract.model.PlanOperationPage;
import io.saas.forge.entitlement.contract.model.PlanPage;
import io.saas.forge.entitlement.contract.model.QuotaDefinitionOperationRecovery;
import io.saas.forge.entitlement.contract.model.QuotaDefinitionOperationPage;
import io.saas.forge.entitlement.contract.model.QuotaDefinitionPage;
import io.saas.forge.entitlement.application.bootstrap.QuotaDefinitionQueries;
import io.saas.forge.entitlement.application.bootstrap.RecoverableQuotaDefinitionService;
import io.saas.forge.entitlement.application.bootstrap.PlanResult;
import io.saas.forge.entitlement.application.bootstrap.QuotaDefinitionResult;
import io.saas.forge.entitlement.application.subscription.CreateInitialSubscriptionService;
import io.saas.forge.entitlement.application.subscription.InitialSubscriptionResult;
import io.saas.forge.entitlement.contract.api.PlatformEntitlementBootstrapApi;
import io.saas.forge.entitlement.contract.model.CreatePlanRequest;
import io.saas.forge.entitlement.contract.model.CreateQuotaDefinitionRequest;
import io.saas.forge.entitlement.contract.model.Plan;
import io.saas.forge.entitlement.contract.model.PlanQuotaLimit;
import io.saas.forge.entitlement.contract.model.PlanStatus;
import io.saas.forge.entitlement.contract.model.QuotaDefinition;
import io.saas.forge.entitlement.contract.model.QuotaDefinitionStatus;
import io.saas.forge.entitlement.contract.model.Subscription;
...
public class EntitlementBootstrapController implements PlatformEntitlementBootstrapApi {
    ...
    private final io.saas.forge.entitlement.application.bootstrap.RecoverablePlanService recoverablePlans;
    private final io.saas.forge.entitlement.application.bootstrap.PlanQueries planQueries;
    private final io.saas.forge.entitlement.application.subscription.RecoverableSubscriptionService initialSubscriptions;
    private final io.saas.forge.entitlement.application.subscription.SubscriptionQueries subscriptionQueries;
    ...
    public EntitlementBootstrapController(
            ...
            io.saas.forge.entitlement.application.bootstrap.RecoverablePlanService recoverablePlans,
            io.saas.forge.entitlement.application.bootstrap.PlanQueries planQueries,
            io.saas.forge.entitlement.application.subscription.RecoverableSubscriptionService initialSubscriptions,
            RecoverableQuotaDefinitionService recoverableQuota,
            QuotaDefinitionQueries quotaQueries,
            io.saas.forge.entitlement.application.subscription.SubscriptionQueries subscriptionQueries) {
    ...
    public ResponseEntity<QuotaDefinitionPage> listQuotaDefinitions(
        ...
        var page = quotaQueries.list(code, status == null ? null
                : io.saas.forge.entitlement.domain.quota.QuotaDefinitionStatus.valueOf(status.name()), cursor, limit);
    ...
    public ResponseEntity<PlanPage> listPlans(
        ...
        var page = planQueries.list(code, status == null ? null
                : io.saas.forge.entitlement.domain.plan.PlanStatus.valueOf(status.name()), cursor, limit);
        return ResponseEntity.ok().cacheControl(org.springframework.http.CacheControl.noStore()).body(
                new PlanPage(
                        page.items().stream().map(EntitlementBootstrapController::toResponse).toList(),
                        page.nextCursor(), page.hasMore()));
    ...
    public ResponseEntity<Plan> createPlan(UUID idempotencyKey, CreatePlanRequest request) {
        ...
        if (request.getQuotaLimits() == null || request.getQuotaLimits().size() != 1 || request.getQuotaLimits().get(0) == null) {
            throw new io.saas.forge.entitlement.domain.plan.PlanInvalidException(
                    "Plan 必须恰好包含一个 max_users 限额");
        }
        var quotaLimit = request.getQuotaLimits().get(0);
        PlanResult result = recoverablePlans.create(
                actor, idempotencyKey, new io.saas.forge.entitlement.application.bootstrap.PlanDraft(request.getCode(), request.getDisplayName(),
    ...
    @Override
    public ResponseEntity<io.saas.forge.entitlement.contract.model.TenantSubscription> getTenantSubscription(UUID tenantId) {
        authorizer.authorize(currentRequest().getHeader(HttpHeaders.AUTHORIZATION));
        var value = subscriptionQueries.get(tenantId);
        var response = new io.saas.forge.entitlement.contract.model.TenantSubscription(
    ...
    public ResponseEntity<Subscription> createInitialSubscription(
            UUID tenantId,
            UUID idempotencyKey,
            io.saas.forge.entitlement.contract.model.CreateInitialSubscriptionRequest request) {
        HttpServletRequest httpRequest = currentRequest();
        UUID actor = authorizer.authorize(httpRequest.getHeader(HttpHeaders.AUTHORIZATION));
