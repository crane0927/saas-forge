...
import io.saas.forge.entitlement.application.bootstrap.PlanRecoveryRepository;
import io.saas.forge.entitlement.application.bootstrap.PlanResult;
import io.saas.forge.entitlement.application.bootstrap.PlanRecoveryException;
import io.saas.forge.entitlement.infrastructure.persistence.mapper.PlanRecoveryMapper;
import io.saas.forge.entitlement.infrastructure.persistence.record.PlanRecoveryRow;
...
public class MyBatisPlanRecovery implements PlanRecoveryRepository {
    ...
    @Override
    public Entry prepare(UUID actor, UUID key, PlanOperation.Operation operation, UUID targetId, io.saas.forge.entitlement.application.bootstrap.PlanDraft draft, Instant now) {
    ...
    private Entry entry(PlanRecoveryRow row) {
        return new Entry(row.id(), row.actor(), row.key(), PlanOperation.Operation.valueOf(row.operation()), row.targetId(),
                row.requestBody() == null ? null : json.readValue(row.requestBody(), io.saas.forge.entitlement.application.bootstrap.PlanDraft.class),
