...
import io.saas.forge.entitlement.application.subscription.SubscriptionRecoveryRepository;
import io.saas.forge.entitlement.application.subscription.InitialSubscriptionResult;
import io.saas.forge.entitlement.application.subscription.SubscriptionRecoveryException;
import io.saas.forge.entitlement.infrastructure.persistence.mapper.SubscriptionRecoveryMapper;
import io.saas.forge.entitlement.infrastructure.persistence.record.SubscriptionRecoveryRow;
...
public class MyBatisSubscriptionRecovery implements SubscriptionRecoveryRepository {
    ...
    @Override
    public Entry prepare(UUID actor, UUID key, io.saas.forge.entitlement.application.subscription.SubscriptionDraft draft, Instant now) {
    ...
    public <T> T locked(UUID actor, UUID id, Function<Entry, T> work) {
        return transactions.execute(status -> {
            ...
            if (!mapper.tryLock(id)) {
                throw new io.saas.forge.entitlement.application.bootstrap.IdempotencyRequestInProgressException();
            }
    ...
    private Entry entry(SubscriptionRecoveryRow row) {
        return new Entry(row.id(), row.actor(), row.key(),
                new io.saas.forge.entitlement.application.subscription.SubscriptionDraft(row.tenantId(), row.planId(),
