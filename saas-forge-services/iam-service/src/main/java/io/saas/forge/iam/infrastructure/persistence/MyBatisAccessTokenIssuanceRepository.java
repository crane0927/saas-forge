...
import io.saas.forge.iam.domain.session.DurableRevocation;
import io.saas.forge.iam.infrastructure.persistence.mapper.AccessTokenIssuanceMapper;
import io.saas.forge.iam.infrastructure.persistence.record.AccessTokenIssuanceRow;
...
public class MyBatisAccessTokenIssuanceRepository implements AccessTokenIssuanceRepository {
    ...
    @Override
    public List<AccessTokenIssuance> findUnexpiredByFamilyId(UUID familyId, Instant at) {
        return mapper.findUnexpiredByFamilyId(familyId, IamTime.asOffsetDateTime(at)).stream()
