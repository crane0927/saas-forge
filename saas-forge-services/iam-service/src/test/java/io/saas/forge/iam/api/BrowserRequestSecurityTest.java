...
class BrowserRequestSecurityTest {
    private final BrowserRequestSecurity security = new BrowserRequestSecurity("saas.forge.test");

    @Test
    void acceptsControlledOriginsAndMissingFetchMetadata() {
        MockHttpServletRequest consoleRequest = request("https://console.saas.forge.test", null);
        assertDoesNotThrow(() -> security.requireControlledMutation(
                consoleRequest, "1", BrowserSessionSlot.TENANT));
        MockHttpServletRequest platformRequest = request("https://platform.saas.forge.test", "same-site");
        assertDoesNotThrow(() -> security.requireControlledMutation(
    ...
    void rejectsExternalOriginCrossSiteFetchWrongCsrfAndNonJson() {
        ...
        assertThrows(BrowserRequestRejectedException.class,
                () -> security.requireControlledMutation(
                        request("https://console.saas.forge.test", "cross-site"), "1", BrowserSessionSlot.TENANT));
        assertThrows(BrowserRequestRejectedException.class,
                () -> security.requireControlledMutation(
                        request("https://console.saas.forge.test", "same-site"), "csrf", BrowserSessionSlot.TENANT));
        assertThrows(BrowserRequestRejectedException.class,
                () -> security.requireControlledMutation(
                        request("https://console.saas.forge.test", "same-site"), "1", BrowserSessionSlot.PLATFORM));
        MockHttpServletRequest text = request("https://console.saas.forge.test", "same-site");
