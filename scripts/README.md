...
- `local-service-replacement.sh <replace|status|restore> iam-service`：在不构建应用镜像、不删除卷的前提下，将唯一健康的容器 IAM 与本机 JVM 受控切换。工具只读取受限 Secret，并用现有 Gateway 发现身份只读判断 Nacos 实例；`replace` 失败时自动恢复容器 IAM。
