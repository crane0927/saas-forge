...
- `verify-iam-local-cross-service-e2e.sh`：使用只读 Platform Admin 凭据文件，在受控浏览器中验证 HTTPS Edge → Gateway → Entitlement → 本机 IAM 的 Platform Role gRPC 成功路径；会保留一个专用 DRAFT Quota Definition。
