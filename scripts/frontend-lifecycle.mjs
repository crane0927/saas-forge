const platformPackage = "@saas-forge/platform-console";
const platformPort = 5173;
const startupWindowMilliseconds = 30_000;

/**
 * Platform 生命周期的唯一公开 interface。调用方只选择操作；状态优先级、
 * PID 身份、幂等和失败回滚都封装在 module 内，系统副作用由 Adapter 提供。
 */
export function createPlatformLifecycle({ repositoryRoot, system }) {
  ...
  async function observe() {
    const [pidFile, legacyPidFile, listener] = await Promise.all([
      ...
      system.inspectListener(platformPort),
    ]);
    ...
    if (!isPlatformProcess(process, consoleRoot, record)) {
      return "UNMANAGED";
    }
    ...
    if (
      ...
      listener.port !== platformPort ||
      listener.processGroupId !== record.pid
    ) {
      return "UNMANAGED";
    }
    return (await system.isHttpsReady("platform.saasforge.test"))
      ? "RUNNING"
      : "UNREADY";
  }
  ...
  async function waitForRunning() {
    ...
    while (system.now() < deadline) {
      ...
      if (state === "UNMANAGED" || state === "STALE") {
        throw new Error(`Platform 启动期间进入不安全状态：${state}`);
      }
      await system.wait(250);
    }
    throw new Error("Platform 未在 30 秒内通过正式 HTTPS 入口就绪。");
  }
  ...
  async function start() {
    ...
    if (initialState === "RUNNING") {
      await system.preflight({ allowManagedPlatform: true });
    ...
    if (initialState !== "STOPPED") {
      throw new Error(`Platform 当前状态为 ${initialState}，拒绝启动。`);
    }
    await system.preflight({ allowManagedPlatform: false });
    const spawned = await system.spawnPlatform({
      logMode: 0o600,
      append: true,
    });
    const record = {
    ...
    try {
      ...
      if (
        current !== undefined &&
        isPlatformProcess(current, consoleRoot, record)
      ) {
        await system.terminatePlatform(record, "SIGTERM");
  ...
  async function stop() {
    ...
    if (initialState === "UNMANAGED") {
      throw new Error("Platform 当前状态为 UNMANAGED，拒绝发送信号。");
    }
    if (initialState === "STOPPED") {
    ...
    if (
      record === undefined ||
      process === undefined ||
      !isPlatformProcess(process, consoleRoot, record)
    ) {
      throw new Error("Platform 进程身份在停止前发生变化，拒绝发送信号。");
    }
    await system.terminatePlatform(record, "SIGTERM");
    for (let attempt = 0; attempt < 40; attempt += 1) {
      if ((await system.inspectProcess(record.pid)) === undefined) break;
      if (attempt === 39) {
        throw new Error("Platform 未在 10 秒内响应 SIGTERM；未发送更强信号。");
      }
      await system.wait(250);
    }
    await system.removePidFile();
    ...
    const state = await observe();
    if (state !== "STOPPED") {
      throw new Error(`Platform 停止后状态为 ${state}，拒绝继续清理。`);
    }
  ...
  return {
    async run(operation) {
      ...
      throw new Error(`不支持的 Platform 生命周期操作：${operation}`);
    },
  };
}
